...
mod tests {
    ...
    fn example_first_try() {
        ...
        assert_eq!(Solution::delete_duplicates(ListNode::generate(vec![1, 1, 1])), ListNode::generate(vec![]));
        assert_eq!(Solution::delete_duplicates(ListNode::generate(vec![1, 1, 2])), ListNode::generate(vec![2]));
        assert_eq!(Solution::delete_duplicates(ListNode::generate(vec![1, 1, 2, 3, 3])), ListNode::generate(vec![2]));
        assert_eq!(Solution::delete_duplicates(ListNode::generate(vec![1, 2, 3, 3, 4, 4, 5])), ListNode::generate(vec![1, 2, 5]));
        assert_eq!(Solution::delete_duplicates(ListNode::generate(vec![1, 1, 1, 2, 3])), ListNode::generate(vec![2, 3]));
    }
